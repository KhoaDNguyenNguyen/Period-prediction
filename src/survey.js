...
function buildOutput(data) {
  ...
  const basic_profile = {
    age: n(data.age),
    height_cm: n(data.height_cm),
    weight_kg: n(data.weight_kg),
    menarche_age: n(data.menarche_age),
    parity: n(data.parity),
  };

  const cycle_history = Array.isArray(data.cycle_history)
    ? data.cycle_history.map((p) => ({
        cycle_start: n(p.cycle_start),
        cycle_length: n(p.cycle_length),
      }))
    : [];

  const lifestyle = {
    sleep_hours: n(data.sleep_hours),
    bedtime: n(data.bedtime),
    wake_time: n(data.wake_time),
    physical_activity: n(data.physical_activity),
    diet: n(data.diet),
    alcohol_per_week: n(data.alcohol_per_week),
    coffee_per_day: n(data.coffee_per_day),
    smoking: data.smoking ?? null,
    shift_work: data.shift_work ?? null,
  };

  const medical_factors = {
    hormonal_birth_control: {
      using: data.hbc_using ?? null,
      type: n(data.hbc_type),
      duration_months: n(data.hbc_duration_months),
    },
    diagnosed_conditions: Array.isArray(data.diagnosed_conditions)
  ...
  const medications_recent_changes = {
    current_drugs:
      data.current_drugs_using === true && Array.isArray(data.current_drugs_list)
  ...
  return {
    ...
    menstruation_symptoms: Array.isArray(data.menstruation_symptoms)
      ? data.menstruation_symptoms
      : null,
...
function displayResults(sender) {
  ...
  fetch(`${API}/survey`, {
    ...
    .then((res) => {
      //1. session expired (401/403) → clear localStorage, alert, redirect to login
      if (res.status === 401 || res.status === 403) {
        localStorage.clear();
        alert('Phiên làm việc đã hết hạn, vui lòng đăng nhập lại.');
        return window.location.replace('/login.html');
      }
      // 2. invalid data (400) → alert, ném lỗi
      if (res.status === 400) {
        alert('Dữ liệu không hợp lệ. Vui lòng thử lại.');
        throw new Error('Bad Request');
      }
      // 3. unexpected error (500) → alert, ném lỗi
      if (!res.ok) {
        throw new Error('Lưu database thất bại');
      }
      // 4. success (200) → parse response
      return res.text().then((t) => (t ? JSON.parse(t) : {}));
    })

    .then(() => {
      document.getElementById('surveyResults').textContent = JSON.stringify(out, null, 2);
    })
    .catch((err) => alert(err.message));
}
surveyModel.onComplete.add(displayResults);

const renderSurvey = () => surveyModel.render(document.getElementById('surveyContainer'));
